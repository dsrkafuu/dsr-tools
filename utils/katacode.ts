/*! KataCode Encoder | DSRKafuU (https://dsrkafuu.net) | Copyright (c) MIT License */
...
export async function encodeKata(str: string) {
  if (typeof str !== 'string') {
    throw new Error('KATA CODE ONLY CAN ENCODE STRING');
  }
  str = str.trim();
  if (str === '') {
    return '';
  ...
  for (const char of str) {
    ...
    if (!codePoint) {
      throw new Error('UNABLE TO ENCODE KATA CODE');
    }
    // 转换为 35 进制数值
...
export async function decodeKata(str: string) {
  if (typeof str !== 'string') {
    throw new Error('KATA CODE ONLY CAN DECODE STRING');
  }
  str = str.trim();
  if (str === '') {
    return '';
  ...
  return res
    .map((val) => {
      ...
      try {
        ...
        throw new Error('UNABLE TO DECODE KATA CODE');
      }
    })
    .join('');
}
